...
if(successCode<0)
{
sprintf(message,"Unable to bind with port %d",this->portNumber);
printf("%s\n",message);
WSACleanup();
return;
}
...
while(1)
{
sprintf(message,"TM server is ready to accept the request on Port %d",this->portNumber);
printf("%s\n",message);
...
}// infinite loop end by forwarding
closesocket(serverSocketDescriptor);
WSACleanup();
return;
}